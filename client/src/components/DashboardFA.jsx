...
const DashboardFA = () => {
  ...
  const fetchForms = async () => {
    try {
      const response = await axios.get("http://localhost:8888/api/faAllForms/");
  ...
  const handleApprove = async (formId) => {
    try {
      const response = await axios.post(
        `http://localhost:8888/api/faApproval/`,
  ...
  useEffect(() => {
    fetchForms();
  }, []); // Empty dependency array ensures that fetchForms is only called once on component mount
